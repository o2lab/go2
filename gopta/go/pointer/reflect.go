...
//TODO: bz: https://golang.org/pkg/reflect/#Value.Call
// panic@google.golang.org/grpc.test: not a tagged object: n3 = xxx when solving constraint n474170 = reflect n473716.Call(n474172)
// see my_log_panic-panic-report2
// according to the doc, Value (a.k.a., x in the following code) should be tagged function, not tagged object
// SOlUTION: change a.taggedValue to a.taggedFunc
func (c *rVCallConstraint) solve(a *analysis, delta *nodeset) {
	...
	for _, x := range delta.AppendTo(a.deltaSpace) {
		vObj := nodeid(x)
		tDyn := a.taggedFunc(vObj) //bz: see above
		if tDyn == nil {
		...
		pkg := tSig.Recv().Pkg()
		if pkg != nil && pkg.Name() == "reflect" {
			continue //bz: this is a reflection function call, theoretically should not appear here
		}
		if tSig.Recv() != nil {
			//TODO: bz: this vObj is not from rtypeMethodByNameConstraint
			// (generated by reflection functions, e.g., Method(), MethodByName()), but they should be
			// SOLUTION -> skip this
			//panic(tSig) // TODO(adonovan): rethink when we implement Method() //bz: original code
			continue
		}

		// Add dynamic call target.
		fn := vObj //no need to check
		...
		for i, n := 0, mset.Len(); i < n; i++ {
			...
			if c.name == "" || c.name == sel.Obj().Name() {
				...
				if isIface {
					...
					//TODO: bz: isIface is not enough to separate all fn/signature with receivers,
					// this will lead to panic in rVCallConstraint @go/pointer/reflect.go:265
					// SOLUTION -> add changeRecv when sig.receiver != nil
					if sig.Recv() != nil {//bz: this also needs to transfer receiver to params
						sig = changeRecv(sig) // move receiver to params[0]
					}
				} else {
...
func ext۰reflect۰rtype۰Method(a *analysis, cgn *cgnode) {
	...
	// so we don't specialize that case.d
	a.addConstraint(&rtypeMethodByNameConstraint{
		cgn:    cgn,
		t:      a.funcParams(cgn.obj),
		result: a.funcResults(cgn.obj),
