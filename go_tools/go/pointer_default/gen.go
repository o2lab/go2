...
func (a *analysis) setValueNode(v ssa.Value, id nodeid, cgn *cgnode) {
	...
	//bz: my code to record all queries
	if a.config.DoRecordQueries {
		t := v.Type()
		//same as api.go and below
		if CanPoint(t) {
		...
		}else if _, ok := v.Type().Underlying().(*types.Pointer); ok && CanPoint(mustDeref(t)){
