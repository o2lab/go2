...
executable lives at github.tamu.edu/April1989/go_tools/cmd/godoc.
...
```
godoc -templates=$GOPATH/src/github.tamu.edu/April1989/go_tools/godoc/static -http=:6060
```
...
2) Run `go generate github.tamu.edu/April1989/go_tools/godoc/static` so `static/static.go` picks
up the change.

3) Run `go install github.tamu.edu/April1989/go_tools/cmd/godoc` so the compiled `godoc` binary
picks up the change.
